chmod +x Nova-Installer.x86_64 #autorise Nova-Installer.x86_64 à être éxécuté en double clique

sudo cp -i Nova-Installer.x86_64 ~/.local/bin #bouge Nova-Installer.x86_64 dans /home/$USER/.local/bin (pour que le .desktop trouve super vite et universelement)

mkdir ~/.local/share/icons/Nova-Installer #créer un dossier pour les icones
cd data
cp icon.png ~/.local/share/icons/Nova-Installer #bouge l'icone dans le bon dossier
cd -

touch Nova-Installer.desktop
echo [Desktop Entry] >> Nova-Installer.desktop
echo Name="Nova Installer" >> Nova-Installer.desktop
echo Exec=~/.local/bin/Nova-Installer.x86_64 >> Nova-Installer.desktop
